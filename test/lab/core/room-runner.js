...
async function runOneLabGame(cfg) {
  ...
  if (cfg.loverLocked) room.loverLocked = !!cfg.loverLocked; // A/B 注入（M3.5）：解绑禁用（G3）
    ...
    return {
      ...
      voteAudit: (global._voteAudit && global._voteAudit.length) ? global._voteAudit.splice(0).map(x => ({ ...x, gameId: cfg.seed || null })) : null, // 1.7.18：投票审计（LAB_AUDIT_VOTE=1，含 25 维在线特征——v3 重训数据源；gameId 用于 group-wise 划分）统一写盘，防多 worker 竞态）
