...
function analyze(recs) {
  ...
  o.thirdWin = o.loverWin.third || 0;
  return o;
}
console.log('=== 神眷者（人狼恋）格局分析 ===');
let Tn = 0, Tlover = 0, Tthird = 0;
for (const name of Object.keys(byName)) {
  const a = analyze(byName[name]);
  Tn += a.n; Tlover += a.lover; Tthird += a.thirdWin;
  console.log(`\n${name}（${a.n}局）`);
  console.log(`  人狼恋发生率: ${(a.loverPct = a.lover / a.n * 100).toFixed(1)}% (${a.lover}/${a.n}) | 神眷者胜率: ${(a.thirdWin / a.n * 100).toFixed(2)}%`);
  if (a.lover) console.log(`  人狼恋局胜负: 好人 ${(a.loverWin.good / a.lover * 100).toFixed(1)}% | 狼人 ${(a.loverWin.wolf / a.lover * 100).toFixed(1)}% | 神眷者 ${(a.loverWin.third / a.lover * 100).toFixed(1)}%`);
...
console.log(`\n=== 全局汇总（${Tn}局）===`);
console.log(`人狼恋发生率: ${(Tlover / Tn * 100).toFixed(1)}% | 神眷者总胜率: ${(Tthird / Tn * 100).toFixed(2)}%`);
const all = Object.values(byName).flat();
const w = w => all.filter(r => r.result.winner === w).length;
console.log(`全局阵营胜率: 好人 ${(w('good') / Tn * 100).toFixed(1)}% | 狼人 ${(w('wolf') / Tn * 100).toFixed(1)}% | 神眷者 0% | 平局 ${(w('draw') / Tn * 100).toFixed(1)}%`);
