...
LOG = os.path.join(OUT, 'hook20.log')
...
function processSong(vf) {
  ...
  flush();
  const sec = (Date.now() - Date.now() + 0.001);
...
Interceptor.attach(rfAddr, {
  onEnter(args) {
    ...
    if (vf.equals(curVf)) { this.blocked = false; return; }
...
send({ type: 'log', msg: 'v20 ready rf=' + rfAddr + ' seek=' + seekAddr });
'''
try:
