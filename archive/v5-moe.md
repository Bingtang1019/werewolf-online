...
### MoE B（策略层）配对验收（52,167 样本 / 2000 局，按对局分组，留出 400 局 / 10,450 样本）
| 方案 | AUC | Brier | LogLoss |
|---|---|---|---|
| v3v3 AdaBoost（A2 同构，Platt 校准） | 0.8053 | 0.1806 | 0.5354 |
| π 意图版（A5 同构） | **0.8645** | 0.1632 | 0.5065 |
| 50:50 平均 | 0.8506 | 0.1662 | 0.5090 |
| 训练集网格最优静态（wAda=0 / wPi=1） | 0.8645 | 0.1632 | 0.5065 |
| logit 堆叠门控（学习型） | 0.8633 | **0.1499** | **0.4616** |

结论：策略层融合**不提升排序**（AUC 与 π 单专家持平/略低，π 占绝对主导），但堆叠门控显著改善**概率校准**（LogLoss −0.0449 / Brier −0.0133）。裁决：`calibration-only` —— 下游只消费排序（rollout 贪心）时不值得接线；下游消费概率（信念 0.6/0.4 混入）时可考虑校准版堆叠。默认维持关闭。工具：`tools/ai/eval-strategy-moe.js`（报告 `data/eval-strategy-moe.json`）。
