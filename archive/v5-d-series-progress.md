...
- **D 生产在线采集**：在需要积累 V5 样本的服务器设置 `V5_SAMPLES=1`，vote 样本会以 21 维 `voteFeaturesV5` 落盘并标记 `v5:true`；不影响默认生产行为。
- **E A4 真人局 A/B**：在目标房间服务器设置 `V5_INTENT_TALK=1` 开启意图感知回应；对比开关关闭/开启时的发言自然度与玩家反馈。
- 两者均为实验开关，未默认开启；待真实数据/反馈后决定是否转正。
