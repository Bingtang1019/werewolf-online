...
//   - 规则式贝叶斯（可解释、零训练成本、权重手调）→ D2 学习式信念整合替代
...
//
// 1.7.17（D1）：信念状态取代特征快照的第一步——死亡因果/查验验证/票型时序三证据源

const W = {
  deathFact: 1.0,      // 客观：死因（wolf/poison/exile 确定性）
  deathInfer: -0.7,    // 1.7.17 方向修复：被刀者的投票目标嫌疑-（被刀者投的人≠狼——被集火/带节奏的是好人；方向审计 AUC 0.47→0.57）
  killMute: -1.2,      // 1.7.17 方向修复：被刀者的查杀声明目标嫌疑-（声明主要来自狼悍跳——被查杀者反而是好人）
  checkFact: 1.0,      // 客观：放逐者身份验证查杀/金水
  voteFlow: -0.5,      // 1.7.17 方向修复：A 投 B → B 嫌疑-（被投多=被集火的好人；同质生态跟票集中在"嫌疑最高者"——但那是带节奏目标，非狼）
  voteRatio: -0.6,     // 1.7.17 方向修复：票型相对领先 → 嫌疑-（同上）
  leadWeight: 0.3,     // 带节奏者识别：先投票者权重（二阶信念）
  claim: -0.3,         // 1.7.17 方向修复：查杀声明 → 目标嫌疑-（当前数据声明主要来自狼悍跳——被查杀者反而是好人；真预言家只遗言报查验）
  clip: [0.05, 0.95],  // 后验压缩
};

// 1.7.17（方向审计结论）：每个证据源的更新方向都反了——
//   票型（被投多=被集火的好人）、声明（狼悍跳假查杀）、死亡推断（被刀者投的人≠狼）
//   逐项反转后 AUC 0.40 → 0.61（+21pp）。
// 历史教训：原 0.6018 是"方向反 × exp-belief 公式反"的双重抵消假象（公式修复后暴露 0.40，
//   方向修复后回到 0.61 真值）——两个 bug 同源（都源自对同质生态投票语义的误解）。
